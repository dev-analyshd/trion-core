#!/usr/bin/env python3
"""TRION Akashic Index — VeChain Genesis Backfill. Walks blocks via
VeChain's public Thor REST API from block 0 to tip, extracts a 128-dim
vector, POSTs to FAISS."""
import os, sys, json, math, time, hashlib, logging, argparse, requests
from concurrent.futures import ThreadPoolExecutor, as_completed

logging.basicConfig(level=logging.INFO, format="%(asctime)s [VECHAIN-BACKFILL] %(message)s")
...
def get_latest_number() -> int:
    try:
        r = requests.get(f"{API}/blocks/best", timeout=10).json()
        return int(r.get("number", 0))
    except Exception as e:
        logger.warning("Could not fetch best block: %s", e)
        return 0


def get_block(number: int):
    try:
        r = requests.get(f"{API}/blocks/{number}", timeout=15)
...
def extract_features(block: dict):
    ...
    if n == 0:
        return None
    gas_used = float(block.get("gasUsed", 0) or 0)
...
def ingest_block(number: int):
    ...
    block_id = block.get("id", str(number))
    ts = block.get("timestamp", 0)
    entity_id = f"vechain-mainnet_block_{number}"
    bh_id = hashlib.sha3_256(("vechain-mainnet" + str(number) + block_id).encode()).hexdigest()
    payload = {
...
def run_backfill(start: int, end: int):
    checkpoint_file = "genesis_backfill_checkpoint_vechain-mainnet.json"
...
if __name__ == "__main__":
    ...
    if end == 0:
        logger.error("Could not reach VeChain Thor API — skipping this cycle.")
        sys.exit(0)
