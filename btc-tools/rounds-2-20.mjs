/**
 * 20-ROUND LOOP — Rounds 2-20 (continuing from round 1 which completed).
 */
...
async function sendTx(label, calls) {
  for (let attempt = 1; attempt <= 60; attempt++) {
    try {
      ...
      if (attempt < 60 && /estimateFee|felt overflow|fetch failed|429|Insufficient|rate lim/i.test(String(e.message))) {
