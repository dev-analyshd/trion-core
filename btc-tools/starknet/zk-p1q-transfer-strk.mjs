// Transfer 50 STRK from OLD deployer to NEW_ADDR (fresh v3-compatible address)
...
// transfer(to, amount_low, amount_high) — 50 STRK = 50 * 10^18
...
console.log('\n=== Submit v3 invoke: transfer 50 STRK to NEW_ADDR ===');
const invokeReq = {
    ...
    if (rcpt.result) {
      ...
      if (rcpt.result.execution_status !== 'SUCCEEDED') {
        ...
        if (balR.result) {
          ...
          console.log('  NEW_ADDR STRK balance:', (Number(bal) / 1e18).toFixed(4));
        }
      }
      break;
    }
