...
func TestDualStrandSignatures(t *testing.T) {
        ...
        for _, payload := range payloads {
                ...
                // Both must be 64-char hex (SHA3-256)
                ...
                // Full canonical invariant (cross-language BH construction):
                // antisense == SHA3-256(p||0xFF) XOR NOT(sense)
                if !DualStrandVerifyPayload(payload, sense, antisense) {
                        t.Errorf("payload=%q: DualStrandVerifyPayload failed — antisense is not the canonical XOR-NOT complement", payload)
                }
...
// TestDualStrandGoldenVectors pins the canonical cross-language dual-strand
// construction to fixed vectors computed with Python's hashlib.sha3_256
// (core/primitives/behavioral_hash.py::hash_dna):
//
//      sense     = SHA3-256(payload || 0x00)
//      antisense = SHA3-256(payload || 0xFF) XOR NOT(sense)
//
// The same vectors are reproducible from Python:
//
//      sense, antisense = hash_dna(payload)
//
// and the invariant sense XOR antisense == NOT(SHA3-256(payload||0xFF))
// is asserted in-line for each vector.
func TestDualStrandGoldenVectors(t *testing.T) {
        t.Log("\n──────────── §2b  DualStrand Golden Vectors — XOR-NOT complement ─────")
        ...
        vectors := []struct {
                ...
                {
                        name:      "uniswap attestation",
                        payload:   []byte(`{"entity":"0xUNISWAP","C":0.72}`),
                        sense:     "e30fd17eac5cd011bca570643c94b6549d25026e773289a4a757ac0f4587c5a5",
                        antisense: "26e701ffa6e860f43a43149804b5aedba4e4aef34f5e8f48550e886f06fb48a9",
                },
                {
                        name:      "meshsha3 invariant payload",
                        payload:   []byte("TRION Behavioral Hash test payload"),
                        sense:     "353f0cfd815ce3057ca934f37347efa53314666463e97bd2588d0846c24f4243",
                        antisense: "24843a8892a6528cf1105f10be859ff4bef361edba62b5a9d87c1d50f6034d37",
                },
                {
                        name:      "empty payload",
                        payload:   []byte{},
                        sense:     "5d53469f20fef4f8eab52b88044ede69c77a6a68a60728609fc4a65ff531e7d0",
                        antisense: "e6e7308c113851eb48835b6e254c1bb5842517b89edaf16aec9f33b7e422bc6d",
                },
                {
                        name:      "hello world",
                        payload:   []byte("hello world"),
                        sense:     "3e8c5e237b93e32efa2c484f7b439eceb6529a8b25d0c509252f34e774de82ab",
                        antisense: "7cfe37ff352e0a587b49e57c9029c67847ae0b34d879fc4395e5f9751ac333da",
                },
        }

        for _, v := range vectors {
                sense, antisense := DualStrandSign(v.payload)

                if sense != v.sense {
                        t.Errorf("%s: sense mismatch\n  got  %s\n  want %s", v.name, sense, v.sense)
                }
                if antisense != v.antisense {
                        t.Errorf("%s: antisense mismatch\n  got  %s\n  want %s", v.name, antisense, v.antisense)
                }
                // Canonical invariant: sense XOR antisense == NOT(SHA3-256(p||0xFF))
                if !DualStrandVerifyPayload(v.payload, sense, antisense) {
                        t.Errorf("%s: sense XOR antisense != NOT(SHA3-256(p||0xFF)) — invariant violated", v.name)
                }
                // Tamper detection: flipping one antisense byte must break verification
                ab, err := hex.DecodeString(antisense)
                if err != nil {
                        t.Fatalf("%s: antisense not valid hex: %v", v.name, err)
                }
                ab[0] ^= 0x01
                tampered := hex.EncodeToString(ab)
                if DualStrandVerifyPayload(v.payload, sense, tampered) {
                        t.Errorf("%s: tampered antisense accepted — complement binding broken", v.name)
                }

                t.Logf("  %-26s sense=%s… antisense=%s… ✅", v.name, sense[:16], antisense[:16])
        }

        t.Logf("  ✅ PASS — golden vectors match Python hashlib.sha3_256; XOR-NOT invariant holds")
}
