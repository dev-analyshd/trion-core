...
**Task ID:** 9-c (Master Command §5/§29) · **Repo:** /home/z/trion-core @ c0ccb14 + this session's fix waves (working tree, coordinator commit pending) + the Wave-5 lane edits (Tasks 11-a…11-d, in-tree at HEAD af5317b+ — the M-073 signal-taxonomy amendment in Path 1 and the R-14 TON/R-21 cosmos notes are the Wave-5 refresh, Task 12-b)
...
- Python streamer: `core/realtime/bh_streamer.py` — 61 EVM + 14 non-EVM family fetchers (TON/cosmos/πXRPL/MX/Hedera… read the same real chain hashes the Rust fix waves now use; TON is now per-seqno `getBlockHeader` root_hash and cosmos reads the current block's `block_id.hash` — R-14/R-21 closed by 11-c, Wave 5, pinned by tests/unit/test_bh_streamer_fetchers.py).

**VALIDATION**
- Field reads per chain (block hash / timestamp / magnitude decimals) per `docs/audit/canonical-sweep/SWEEP-B.md` §1 table; missing block hash → warn + honest `"0x0"` (CANONICAL_BH §9 — enforced this session for ton/pi/xrpl/mx (7-d) and hedera (8-b), and extended to vechain/algorand/cardano by the parallel lane; re-verified in-tree at the 12-b refresh: all three crates read the real chain hash and warn+"0x0" only when genuinely missing).
...
- Φ(t) from the 9 entropy features; `anima-service` `_compute_signal` builds plane values; `core/master/signal_factory.py build_signal` (34-field TRIONSignal schema; canonical 29-type taxonomy on a 24-id fork-gated registry — see the Wave-5 amendment below) applies the master equation T(t)=[C(t)≥Θ(t)]·S(t)·e^(M_moat·t); SILENCE payloads now derive limiting_plane/eta (M-004 fix, 8-c: `_derive_silence_payload` at signal_factory.py:334, `ETA_BLOCKS_PER_GAP=1000` at :321); validator provenance layer 5 (`_load_validator_registry`/`_resolve_validator_figures`, :410/:435 — registry-first, no fabrication).
- AWA emission gate (`core/governance/awa`) can freeze ALL emission (503).

**Wave-5 amendment (M-073 owner ruling — Task 11-a):** the canonical signal taxonomy is **29 types** = 19 base (D1 §11) + 10 BTCP-family (BTCP master spec §2 + §14.2); the closed set holds **27 distinct names** (BTCP_ROUTE and CONSENSUS_ADAPTATION are counted in both families). The registry **id space stays fork-gated at 24** (ids 0–23 dense — wasm `signal_type_count()==24`, rust `SIGNAL_TYPE_COUNT==24`, on-chain packed ids 0–23: cross-language parity preserved, new ids require a protocol fork). The 7 BTCP-family names that are not enum members (BEHAVIORAL_TRUTH, SHADOW_CHAIN, LIQUIDITY_OCEAN, CHAIN_RELIABILITY, BTCP_ESCROW_EVENT, BTCP_TIMEOUT, GENESIS_COMMITMENT) ride their closest **canonical carrier** as typed `signal_subtype` payloads, each with a spec-faithful builder + fail-closed validation in `core/master/signal_factory.py`; `signal_registry()` exposes the ruling view (canonical_total 29 / closed_set 27) alongside the id-space view. Pinned by 20 new tests in `tests/unit/test_signal_registry.py` (full tests/unit 1107P/9S at 11-a close); the ruling + invariants are recorded in `spec/signal_types.md`.
...
- Unchanged pre-existing bugs on these paths were fixed by a parallel lane during this session (verified, pinned): `/api/v1/signal/type/*` COLD_START KeyError (api/app.py:6017) and the GOVERNANCE_SIGNAL digest-overflow IndexError (api/app.py:6137) — regression tests/unit/test_api_cold_start.py (8 passed at write time; all 19 signal types answer 200 on a cold entity). The Python-side APTOS fetcher residual noted at write time (stale `previous_block_hash` key → "0x0") was closed by 10-a (R-15: the fetcher now reads the live `block_hash` field) and the TON/cosmos fetcher divergences were closed by 11-c (R-14/R-21, see Path 1 ENTRY — the remaining per-tx rows on those non-EVM Python paths are still synthetic, the Rust crates are the real per-tx path).
