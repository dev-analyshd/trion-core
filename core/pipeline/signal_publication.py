        ...
        if self.chain_relay is not None and self.chain_relay.ready:
            try:
                if signal["signal_emitted"] or force_publish:
                    # ── Gap #5: typed emission via publishSignalWithType ─────────
                    # The previous code called publish_behavioral_signal_v3
                    # directly, which hard-coded the on-chain type to
                    # VALUATION. That made it impossible to emit a
                    # MANIPULATION_ALERT / RESURRECTION / TRAJECTORY / …
                    # through the same pipeline. We now route through
                    # publishSignalWithType, which carries the 24-type
                    # taxonomy tag (M-073) on every emission.
                    signal_type_name = "VALUATION"
                    if not signal.get("signal_emitted", False):
                        # Caller forced publish of a non-coherent signal —
                        # treat as SILENCE so the contract records it.
                        signal_type_name = "SILENCE"
                    elif signal.get("manipulation_alert"):
                        signal_type_name = "MANIPULATION_ALERT"

                    if hasattr(self.chain_relay, "publishSignalWithType"):
                        tx = self.chain_relay.publishSignalWithType(
                        ...
                        publication["method"] = "publishSignalWithType"
                    else:
                        # Fallback for older ChainRelay implementations
                        # that have not yet picked up publishSignalWithType.
                    ...
                    publication["signal_type"] = tx.get("signal_type", signal_type_name)
                    self.publication_count += 1
                    logger.info(
                        "Published %s signal for %s: C=%.4f Θ=%.4f coherent=%s tx=%s",
                        publication["signal_type"], entity_id,
                        signal["coherence"], signal["threshold"],
