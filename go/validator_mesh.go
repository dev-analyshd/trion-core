...
func dualStrandSign(payload []byte) (sense, antisense string) {
	s := sha256.Sum256(append(payload, 0x00))
	a := sha256.Sum256(append(payload, 0xFF))
...
func main() {
	...
	profileA := ValidatorProfile{
		ID:               ValidatorID(sha256.Sum256([]byte("validatorA"))),
		Addr:             "127.0.0.1:7001",
	...
	profileB := ValidatorProfile{
		ID:               ValidatorID(sha256.Sum256([]byte("validatorB"))),
		Addr:             "127.0.0.1:7002",
