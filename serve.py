...
# The root-level symlink bh_ledger.db → anima-service/bh_ledger.db is destroyed on
# container reset. Recreate it at every startup so api can always find
# the ledger via the canonical relative path "../bh_ledger.db".
_root     = os.path.dirname(os.path.abspath(__file__))
_link     = os.path.join(_root, "bh_ledger.db")
_target   = os.path.join("anima-service", "bh_ledger.db")   # relative → survives moves
