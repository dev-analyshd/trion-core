 ...
 * ==========================================
 * Polls AlgoNode public REST for rounds and transactions.
 * Produces 128-dim vectors AND per-tx canonical BH (L0.1 ledger).
 *
 * Algorand behavioral dimensions (9 Shannon entropy features):
 *   f1 — Tx type entropy           H(pay/keyreg/axfer/afrz/acfg/appl)
 *   f2 — Sender entropy            H(sender address frequency)
 *   f3 — Receiver entropy          H(receiver frequency)
 *   f4 — Amount entropy            H(amount bins, microAlgos)
 *   f5 — Fee entropy               H(fee bins)
 *   f6 — Asset diversity           H(asset-id frequency)
 *   f7 — App call entropy          H(application-id frequency)
 *   f8 — Close-remainder ratio     H(transfers with close-remainder)
 *   f9 — Group ratio entropy       H(grouped vs atomic tx)
 */
...
async fn algo_get(client: &reqwest::Client, base: &str, path: &str) -> Result<Value> {
    ...
    if !resp.status().is_success() {
        anyhow::bail!("Algod HTTP {} on {}", resp.status(), path);
    }
    Ok(resp.json().await?)
}

async fn get_latest_round(client: &reqwest::Client, base: &str) -> Result<u64> {
    let v = algo_get(client, base, "/v2/status").await?;
    Ok(v.get("last-round").and_then(|r| r.as_u64()).unwrap_or(0))
}

async fn get_block(client: &reqwest::Client, base: &str, round: u64) -> Result<Value> {
    algo_get(client, base, &format!("/v2/blocks/{}", round)).await
}

/// Canonical event-type classification for Algorand transaction types.
fn classify_algorand(tx: &Value) -> u8 {
    ...
    match ttype {
        "pay"   => 0,   // TRANSFER (algo payment)
        "axfer" if has_close => 14, // BURN (asset close-out)
        "axfer" => 0,   // TRANSFER (asset transfer)
        "afrz"  => 4,   // UNSTAKE (asset unfreeze / release)
        "acfg"  => 13,  // MINT (asset create/config)
        "keyreg" => 3,  // STAKE (key registration = participation)
        "appl"  => {
            // Application call — classify by on-completion
            let oc = tx.get("on-completion").and_then(|v| v.as_str()).unwrap_or("noop");
            match oc {
                "optin"  => 11,  // DEPLOY (create/opt-in)
                "closeout" | "clear" => 4, // UNSTAKE
                "create" => 11,  // DEPLOY
                _ => {
                    // Heuristic: app calls to known DEX/DeFi app IDs → SWAP
                    let app_id = tx.get("application-id").and_then(|v| v.as_u64()).unwrap_or(0);
                    if app_id > 0 { 1 } else { 6 } // SWAP or GOVERNANCE
                }
            }
        }
        _ => 0,
...
fn extract_features(txs: &[Value]) -> [f64; 9] {
    ...
    for tx in txs {
        let ttype = tx.get("tx-type").and_then(|v| v.as_str())
            .or_else(|| tx.get("type").and_then(|v| v.as_str()))
            .unwrap_or("pay").to_string();
        ttypes.push(ttype);

        senders.push(tx.get("sender").and_then(|v| v.as_str()).unwrap_or("").to_string());

        let recv = tx.get("payment-transaction")
            .and_then(|p| p.get("receiver").and_then(|r| r.as_str()))
            .or_else(|| tx.get("asset-transfer-transaction")
                .and_then(|a| a.get("receiver").and_then(|r| r.as_str())))
            .or_else(|| tx.get("receiver").and_then(|r| r.as_str()))
            .unwrap_or("");
        receivers.push(recv.to_string());

        let amt = tx_amount_micro(tx) as f64 / MICRO_ALGO;
        if amt > 0.0 { amounts.push(amt); }

        let fee = tx.get("fee").and_then(|f| f.as_u64()).unwrap_or(0) as f64 / MICRO_ALGO;
        fees.push(fee);

        let asset = tx.get("asset-transfer-transaction")
            .and_then(|a| a.get("asset-id"))
            .map(|id| match id {
        ...
        if let Some(app_id) = tx.get("application-id").and_then(|v| v.as_u64()) {
            apps.push(app_id.to_string());
        }

        let has_close = tx.get("payment-transaction")
            .and_then(|p| p.get("close-remainder-amount")).is_some()
            || tx.get("asset-transfer-transaction")
                .and_then(|a| a.get("close-amount")).is_some();
...
static MAX_MICRO: AtomicU64 = AtomicU64::new(1_000_000_000); // 1000 ALGO reference
...
fn algo_bh_batch(txs: &[Value], round: u64, block_hash: &str, ts: u64) -> TxBhBatch {
    ...
    for tx in txs {
        ...
        let selector = tx.get("tx-type").and_then(|v| v.as_str()).unwrap_or("").to_string();
...
async fn main() -> Result<()> {
    ...
    info!("TRION Algorand Rust Indexer — chain={} poll={}ms algod={}", CHAIN_ID, poll_ms, base);

    loop {
        if !faiss.is_healthy().await { sleep(Duration::from_secs(5)).await; continue; }

        let latest = match get_latest_round(&client, &base).await {
            Ok(n) => n,
            Err(e) => { warn!("Algorand latest round error: {}", e); sleep(Duration::from_millis(poll_ms)).await; continue; }
        };
        ...
        for round in from..=latest {
            let block = match get_block(&client, &base, round).await {
                Ok(v) => v,
                Err(e) => { warn!("[{}] round {} error: {}", CHAIN_LBL, round, e); continue; }
            };

            // Block structure: { block: { txns: [...] }, ... }
            let txs: Vec<Value> = block.pointer("/block/txns")
                .and_then(|v| v.as_array())
                .cloned()
                .unwrap_or_default();

            let block_hash = block.pointer("/cert/proposal/ophash")
                .and_then(|v| v.as_str())
                .or_else(|| block.get("hash").and_then(|v| v.as_str()))
                .unwrap_or("")
                .to_string();
            let ts = block.pointer("/block/ts")
                .and_then(|v| v.as_u64())
                .unwrap_or_else(|| {
            ...
            match faiss.add_batch(&payload).await {
                Ok(added) => {
                    ...
                    info!("[{}] round={} txs={} φ={:.4} added={} bh_stored={}",
                          CHAIN_LBL, round, txs.len(), phi, added, bh_stored);
                }
