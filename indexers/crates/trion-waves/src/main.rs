...
async fn main() -> Result<()> {
    ...
    let base      = std::env::var("WAVES_RPC_URL").unwrap_or_else(|_| WAVES_URLS[0].into());
    ...
    loop {
        ...
        let latest = match get_height(&client, &base).await {
            Ok(n) => n,
            Err(e) => { warn!("Waves height error: {}", e); sleep(Duration::from_millis(poll_ms)).await; continue; }
        };
